...
void AsemanAbstractAgentClient::setSocket(AsemanAbstractClientSocket *socket)
{
    ...
    if(p->socket)
    {
        ...
        disconnect(p->socket.data(), &AsemanAbstractClientSocket::answer, this, &AsemanAbstractAgentClient::answer);
    ...
    if(p->socket)
    {
        ...
        connect(p->socket.data(), &AsemanAbstractClientSocket::answer, this, &AsemanAbstractAgentClient::answer);
...
qint64 AsemanAbstractAgentClient::pushRequest(const QString &service, int version, const QString &method, const QVariantList &args, qint32 priority, bool hasResult)
{
    if(!p->socket)
        return 0;
    return p->socket->pushRequest(service, version, method, args, priority, hasResult);
}

...
#define ASEMAN_AGENT_CALLBACK(TYPE) qint64 id, TYPE result, AsemanAbstractAgentClient::CallbackError error
...
public:
    ...
    virtual ~AsemanAbstractAgentClient();
...
public Q_SLOTS:
    virtual qint64 pushRequest(const QString &method, const QVariantList &args, qint32 priority, bool hasResult) = 0;
    qint64 pushRequest(const QString &service, int version, const QString &method, const QVariantList &args, qint32 priority, bool hasResult);
...
protected:
    ...
    void callBackJs(QJSValue jsCallback,const T &result, const CallbackError &error) {
        ...
        errorMap[QStringLiteral("code")] = error.errorCode;
        errorMap[QStringLiteral("value")] = error.errorValue;
        errorMap[QStringLiteral("null")] = error.null;
